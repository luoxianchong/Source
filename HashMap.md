    ...
    final Node<K,V>[] resize() {
        ...
        if (oldTab != null) {
            ...
            for (int j = 0; j < oldCap; ++j) {
                Node<K,V> e;
                if ((e = oldTab[j]) != null) {//如果旧桶不为null，使用e记录旧桶
                    oldTab[j] = null;//旧桶已经取出、将桶置空
                    if (e.next == null)//链表只有一个元素（e本身）
                        newTab[e.hash & (newCap - 1)] = e;
                    else if (e instanceof TreeNode)//是否为红黑二叉树
                        ((TreeNode<K,V>)e).split(this, newTab, j, oldCap);
                    else { 
                    //如果旧桶中的结构为链表,根据变化的最高位的不同，也就是0或者1，表拆分开
...
1、链表长度大于等于8,且不包含数组中的元素，若包含就是9

2、map的容量（数组的长度）需要大于64，小于则进行扩容
