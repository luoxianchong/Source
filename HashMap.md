...
//扩容， 扩容条件：已使用容量size> 阈值（最大容量*加载因子） 并且 当前hash下标的数组元素非空
                ...
                if ((e = oldTab[j]) != null) {//如果旧桶不为null，使用e记录旧桶
                    ...
                    if (e.next == null)//链表只有一个元素（e本身）
                      ...
                      //如果旧桶中的结构为链表,根据变化的最高位的不同，也就是0或者1，表拆分开
                        ...
                        Node<K,V> loHead = null, loTail = null; //旧桶链表,即旧下标的链表 
                        
                        Node<K,V> hiHead = null, hiTail = null;//新桶链表，即旧下标+oldCap(扩容后)的链表
                        Node<K,V> next;
                        do {
                            /*********************************************/
							/**
...
1、链表长度大于等于8,且不包含新加入的元素，若包含就是9，超过8个则变为红黑树
...
### ConcurrentHashMap 5连问

###### 1、扩容过程中，读访问能否访问到数据，如何实现的？

​	主要看  transfer(tab, nextTab)  和  get(key)

​	0、如果在扩容当中读操作有两种情况，1、forwardingNode没有放入tab(就数组)中，2、forwardingNode已经放入就数组中。

​	1、如果没放那就按tab[] 中正常读，如果是链表就读链表，是红黑树（treeBin）则首节点hash为-2,

​    2、如果放置了forwardingNode 则读forwardingNode 首节点hash为-1，然后从forwardingNode 中取出新表。
...
###### 3、指定桶位形成红黑树，且当前红黑树正在自平衡，那此时的读线程是被阻塞等待还是其他方案？详细讲讲





###### 4、jdk8统计散列表中的个数是如何实现的？为什么不使用AtomicLong  

答：使用的是LongAddr 的机制 如果是单线线程 则basCount使用cas加数量，

​	    出现并发则使用counterCells的数组 保存当前线程增加的数量。数组下标是通过当前线程hash&n-1 ,n是counterCells 的数组长度。

​       counterCells 的扩容：counterCells 为空，并且 cellbusy 标志为0（不忙碌），

​       还有 第一次cas basCount 未成功，再次cas bascount 不成功，未发生碰撞，n的长度小于cpu核数。



###### 5、简单讲讲LastRun机制?

lastRun 机制是一个链表上lastRun位置之后的元素hash&n （n为旧数组的长度）的值相同。

在第一个for循环中遍历了全部链表并找到了lastRun的位置。那么便于第二个循环处理。截取lastRun位置。
...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    static final int UNTREEIFY_THRESHOLD = 6;// 由红黑树转化为链表的阈值
    ...
    private transient volatile int sizeCtl;//-1表示正在初始化或扩容，-N 表示有线程正在进行扩容操作，0 表示还没有初始化，正数表示 数组长度*0.75 阈值。
...
当其父节点为红色，叔叔节点是黑色，且当前节点是右子树。进行左旋

##### 红黑树 右旋条件

当其父节点为红色，叔叔节点为黑色，且当前节点是左子树，右旋
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/JdLkEI9sZfdQufI0bWCqsndWGvw2N1m6BZtLh8sVXwo6moaTwh5aOtBVZrpDgVUv9wjeHibNEoB794xhWLctoFQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

JDK 1.8 中，采用Node + CAS + Synchronized来保证并发安全。取消类 Segment，直接用 table 数组存储键值对；当 HashEntry 对象组成的链表长度超过 TREEIFY_THRESHOLD 时，链表转换为红黑树，提升性能。底层变更为数组 + 链表 + 红黑树。

![img](https://mmbiz.qpic.cn/mmbiz_jpg/JdLkEI9sZfdQufI0bWCqsndWGvw2N1m61hmX9mcpcL5U74gIA83JKKQiawWrwy85iaC1Oac8n2AQja0pvMbNEyLw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
